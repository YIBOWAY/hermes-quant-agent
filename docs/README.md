...
**V8-M1 Adversarial Acceptance Prep ACCEPT@2026-07-23**（docs-only；`docs/audits/2026-07-23-v8-m1-adversarial-acceptance-prep.md`）（TC-V8-M1-01…29 + G01…G08 + GAP-01…17 + suite inventory；Architect FREEZE GO；`release_authorized=false`；**Not** full V8 release / canary / public write / M6；NEXT **V8-M2 hermetic adversarial suite close**）。
...
7. **V6 本地 dark enablement + L2a–L5c thin rail：ACCEPT（2026-07-21…22）。** 真实 adapter、
   ...
   projection + L5c workbench a11y）ACCEPT。**不等于** Plan-V6 完整 Web Chat UI / public cutover。V7a–V7g-B-M5 decide/release/stop/projector/Gate-surfaces/typed-results/hermetic+authorized-live-RO Vertical A + hermetic Vertical B bind+plan-confirm+Gate1 seed+Gate1 confirm+Gate2 seed ACCEPT。下一施工：**V8-M2 hermetic adversarial suite close**（V8-M1 prep ACCEPT；token-stream M1 ACCEPT@5788379；M6 未授权；full V8 gates NOT STARTED）；ops restart 纪律；public write 始终 OFF 直至 full V8。
