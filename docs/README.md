...
> 事实快照：2026-07-12。状态变化后优先更新本文件和对应活跃计划，不要在
> `AGENTS.md` 追加开发流水账。
...
| 当前实现 | [`superpowers/plans/2026-07-10-phase-1a-4-v2.md`](superpowers/plans/2026-07-10-phase-1a-4-v2.md) | Slice 9A-9G + mini 9H 已完成；完整 9H cron/notify 是下一待展开切片。 |
| 当前交付记录 | [`superpowers/plans/2026-07-12-slice-9g-opportunity-ledger.md`](superpowers/plans/2026-07-12-slice-9g-opportunity-ledger.md) | 9G 稳定 signal identity、decision/action ledger、精确平台行动关联与 missed 安全判定。 |
| 前序交付记录 | [`superpowers/plans/2026-07-12-slice-9f-mini-9h.md`](superpowers/plans/2026-07-12-slice-9f-mini-9h.md) | 9F proposal-only 市场推演和 `/hermes` 真实只读产物架。 |
...
当前代码交付到 Slice 9G：真实 2026-07-10 options artifact 的 748 个候选中，59 个
阈值信号得到 59 个不碰撞的稳定 ID；因为没有支持的 paper-options route，它们全部诚实
保持 `not_actionable`，不会制造虚假 `missed`。9G 是审计后端/CLI，不是新页面；它没有
新增 `/hermes` 卡片、数据库表、定时调度或通知。

Git、远端和运行进程是易变状态，不在本入口维护“ahead/dirty/尚未推送”快照。每次交接
都应以 `git status --short --branch`、`git log`、`curl /api/health` 和对应测试重新核验。
下一步不是恢复旧 1a-4 模板或旧前端 P2/P3，而是先按当前源码为完整 9H 的 cron、
prediction reconcile、weekly aggregation、notify 与 freshness 另立 bite-sized plan。
...
1. 本文件：确定当前主线和状态。
2. 当前实现计划：只执行其中标明为 active/current 的 slice。
...
- `superpowers/plans/2026-07-10-phase-1a-4-v2.md`：当前实现计划。
...
原 Phase 1a-4 spec/plan 保留为历史输入；`2026-07-10-phase-1a-4-v2.md` 已记录逐项
复审结果和小切片新顺序。9G 已有独立交付计划；完整 9H 仍不得直接凭旧代码模板开工。

### 历史审计

`audits/` 记录特定日期的发现和后续处置。审计中的未完成项、优先级和“下一步”只对
该快照有效；当前状态回到本文件、活跃计划、源码和测试核验。
...
- `/hermes` 已从 Slice 8 骨架升级为 mini 9H 只读产物架；它通过
  `GET /api/hermes/artifacts` 展示风险、预测状态和市场推演。disabled composer、保留
