...
| COO 统一重构（隔离施工） | [`superpowers/specs/2026-08-13-coo-unify-design.md`](superpowers/specs/2026-08-13-coo-unify-design.md) | 只在 `.worktrees/coo-unify/` 的 `refactor/coo-unify` 与空库 `quantplatform_coo` 施工。研究回路可闭环但默认不跑：owner 提出需求后才入队。RD-Agent/Qlib LLM 走 Hermes `grok-4.6`。不改 live soak，不推 `main`。 |
