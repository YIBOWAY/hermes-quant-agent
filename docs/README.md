...
**V8-M2 hermetic adversarial suite close ACCEPT_WITH_NITS@2026-07-23**（platform `f5d41f4`）（GAP-02/08/09/11 COVERED；01/03/07/13 PARTIAL；04/10→M3；bound pytest+vitest green；**Not** cold-start/canary/public/M6/release；NEXT **V8-M3 cold-start/backup-restore drills**）。
...
7. **V6 本地 dark enablement + L2a–L5c thin rail：ACCEPT（2026-07-21…22）。** 真实 adapter、
   ...
   projection + L5c workbench a11y）ACCEPT。**不等于** Plan-V6 完整 Web Chat UI / public cutover。V7a–V7g-B-M5 decide/release/stop/projector/Gate-surfaces/typed-results/hermetic+authorized-live-RO Vertical A + hermetic Vertical B bind+plan-confirm+Gate1 seed+Gate1 confirm+Gate2 seed ACCEPT。下一施工：**V8-M3 cold-start / backup-restore drills**（V8-M1+M2 ACCEPT@f17c27b/`71b6fb6`；token-stream M1 ACCEPT@5788379；M6 未授权；full V8 gates NOT STARTED）；ops restart 纪律；public write 始终 OFF 直至 full V8。
