...
**V8-M4 independent security+code CLEAR_WITH_NITS@2026-07-23**（platform freeze `b570f94`；`docs/audits/2026-07-23-v8-m4-independent-clear.md` + verdict JSON）（G4 checklist PASS；bound campaign platform 357p/5sk + HQA 271p + FE 51；residuals honest；`release_authorized=false`；**Not** canary/public/M6/release；NEXT **V8-M5 canary（USER AUTH REQUIRED）**）。
...
7. **V6 本地 dark enablement + L2a–L5c thin rail：ACCEPT（2026-07-21…22）。** 真实 adapter、
   ...
   projection + L5c workbench a11y）ACCEPT。**不等于** Plan-V6 完整 Web Chat UI / public cutover。V7a–V7g-B-M5 decide/release/stop/projector/Gate-surfaces/typed-results/hermetic+authorized-live-RO Vertical A + hermetic Vertical B bind+plan-confirm+Gate1 seed+Gate1 confirm+Gate2 seed ACCEPT。下一施工：**V8-M5 canary grant（USER AUTH REQUIRED）**（V8-M1–M3 ACCEPT@f17c27b/`f5d41f4`/`b570f94`；**V8-M4 CLEAR_WITH_NITS**@freeze `b570f94`；token-stream M1 ACCEPT@5788379；M4 ≠ M5 auth；M6 未授权；full V8 gates NOT STARTED for release）；ops restart 纪律；public write 始终 OFF 直至 full V8。
