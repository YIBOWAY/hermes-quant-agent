...
7. **V6 本地 dark enablement + L2a–L5c thin rail：ACCEPT（2026-07-21…22）。** 真实 adapter、
   ...
   projection + L5c workbench a11y）ACCEPT。**不等于** Plan-V6 完整 Web Chat UI / public cutover。V7a–V7g-B-M5 decide/release/stop/projector/Gate-surfaces/typed-results/hermetic+authorized-live-RO Vertical A + hermetic Vertical B bind+plan-confirm+Gate1 seed+Gate1 confirm+Gate2 seed ACCEPT。下一施工：**V8-M6 public flag cutover（careful gate under standing safety rails）**（V8-M1–M4 + **V8-M5 ACCEPT@bb67fa3** hermetic canary G5/G6；token-stream M1 ACCEPT@5788379；M5 ≠ M6 auth；M6 Gate2 decide 未授权；full V8 gates NOT STARTED for release）；ops restart 纪律；public write 始终 OFF 直至 full V8 honest。
