...
**Plan-V6 完整 Web Chat UI（token stream）/ V7f+（typed results/verticals）/ launchd 常驻
...
7. **V6 本地 dark enablement + L2a–L5c thin rail：ACCEPT（2026-07-21…22）。** 真实 adapter、
   ...
   projection + L5c workbench a11y）ACCEPT。**不等于** Plan-V6 完整 Web Chat UI / public cutover。V7a–V7e decide/release/stop/projector/Gate-surfaces ACCEPT。下一施工：**V7f typed results** → V7g verticals（token stream 另议）；ops restart 纪律；public write 始终 OFF 直至 V8。
