...
> **状态（2026-07-23）：CURRENT / PLAN ACCEPTED / V0–V5 DONE（V2 live durable OFF）/ V6 LOCAL DARK ENABLEMENT ACCEPT@2026-07-21 / L2a–L5c thin rail ACCEPT@2026-07-22 / V7a–V7g-B-M2 decide/release/stop/projector/Gate-surfaces/typed-results/hermetic+live-RO Vertical A + hermetic Vertical B bind + plan-confirm cascade notch ACCEPT@2026-07-22…23 / Plan-V6 FULL UI PARTIAL / NEXT V7g-B-M3+ explicit Gate1 seed（public write 仍 OFF）。** 本计划是 D-32 唯一 active
...
> token stream / launchd 常驻 / public V8 仍未做（V7a–V7g-B-M2 decide/release/stop/projector/Gate-surfaces/typed-results/hermetic+authorized-live-RO Vertical A + hermetic Vertical B bind+plan-confirm ACCEPT；V7g-B-M3+ Gate1 seed OPEN）。
>
...
| Plan-V6 完整 UI | **PARTIAL / NEXT** | assistant token stream 等仍未做；a11y M1 + V7a–V7g-B-M2 ACCEPT；V7g-B-M3+ Gate1 seed OPEN |
...
- transcript + assistant stream、Task drawer、plan/step、typed result canvas、provider/usage、
  ...
  V7a–V7g-B-M2 decide/release/stop/projector/Gate-surfaces/typed-results/hermetic+authorized-live-RO Vertical A + hermetic Vertical B bind+plan-confirm ACCEPT；token stream / V7g-B-M3+ Gate1 seed 仍未做。
...
**进度（2026-07-23）：** `V7a`–`V7g-B-M2` **ACCEPT** — command-approval decide/release/stop/projector + Domain Gate 1/2/3 surfaces + typed results on spine + **Vertical A bind**（hermetic + authorized live Futu RO）+ **Vertical B hermetic factor bind + plan-confirm**（`vertical.factor_b.bind` → `vertical.factor_b.plan_confirm` CAS → `cascade_stage=plan_confirmed`；lifts only `plan_confirm_required`；`gate_cascade_locked` stays）。无 always-allow；无 public write；不发明 Task from conversation.turn。其余 V7（**V7g-B-M3+ explicit Gate1 seed**）仍 OPEN。
...
| V6 final workspace UI | **PARTIAL（2026-07-23）** | 本地 dark enablement + L2a–L5c thin rail ACCEPT（含 L5b/L5c）；V7a–V7g-B-M2 decide/release/stop/projector/Gate-surfaces/typed-results/hermetic+live-RO Vertical A + hermetic Vertical B bind+plan-confirm ACCEPT；token stream 仍 NEXT；public chat 仍 OFF |
| V7 decisions/results/vertical slices | **PARTIAL（V7a–V7g-B-M2 ACCEPT@2026-07-22…23）** | command-approval decide/release/stop/projector + Gate surfaces + typed results + Vertical A hermetic+live RO + Vertical B hermetic bind+plan-confirm green；**V7g-B-M3+ explicit Gate1 seed** 仍 OPEN |
...
8. **V6 本地 dark + L2a–L5c thin rail ACCEPT（2026-07-21…22）**：真实 HTTP adapter、
   ...
   **Plan-V6 完整 UI 仍 PARTIAL；V7a–V7g-B-M2 decide/release/stop/projector/Gate-surfaces/typed-results/hermetic+authorized-live-RO Vertical A + hermetic Vertical B bind+plan-confirm ACCEPT。** 当前施工入口：**V7g-B-M3+ explicit Gate1 seed act**（still zero orders；kill_switch true；public write OFF；StartResearch 仍 dark；不 auto-seed from plan_confirm）→ optional token stream → V8；public chat 始终 OFF 直至 V8。
