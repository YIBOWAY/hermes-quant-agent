...
> `7d81611`。platform
> `codex/agent-v0-2-platform-v1@bb62f5d0bc6a`（已推送）与 Hermes integration
> `codex/v2-live-integration@2eb5fa27790f`（已推送至用户 fork）。旧 base 字段统一解释为
> `historical_fork_base`：HQA `a7428b6219de`、platform `7b73b5f2fe9e`、Hermes
> `a79b81836070`；它只表示历史祖先，不是 expected head 或 runtime identity。V0 identity、validation
> binding 与 independent `CLEAR` 已闭合，verdict 明确 `release_authorized=false`。V2 source 已独立
...
| HQA Intent/WorkflowAuthority | V3 已交付 AES-GCM/Keychain intent、TTL/tombstone、Task `1:N` Attempt、完整 reducer/CAS/replay/backup 与暗态本地安装 | 尚未由 V4 BFF/PG saga 或 V5 worker 接入；公共 mutation 仍关闭 |
...
| Hermes `/v1/runs` | V2 durable candidate source 已独立 ACCEPT 并推送 | live Hermes 未安装 candidate；controlled install/canary 另授权，当前 write gate OFF |
...
2026-07-19 收口证据为：V3 targeted `218 passed`，HQA full（显式跳过 isolated real-Hermes）
`1726 passed, 15 skipped`；V0/V2 bundle 绑定 platform `1573 passed, 99 skipped`、Hermes focused
`389 passed` 与 HQA V0/V2 `733 passed`。任何新 slice 开始和收口时仍必须重新获取 git、测试、
进程、端口、DB schema、Hermes source/runtime identity，不能把本日期快照当永久当前事实。
...
状态：**SOURCE + FORMAL EVIDENCE + INDEPENDENT CLOSE-OUT DONE。** HQA executable
model/contracts、三仓 identity manifest、fresh exact validation binding 与 independent `CLEAR` 已
交付；verdict 明确 `release_authorized=false`，所以 V0 完成不会自动打开任何 runtime gate。

**V0 closure update（§8.2，2026-07-19）：**

- **formal source snapshot**：HQA `codex/full-9h@4fdad9e7b31b`；
  platform `codex/agent-v0-2-platform-v1@bb62f5d0bc6a` 已推送；Hermes integration
  `codex/v2-live-integration@2eb5fa27790f` 已推送至用户 fork。
- **manifest 边界**：formal artifact 必须在生成瞬间精确声明三仓 tracked/untracked dirty。platform
  主工作树含独立用户/并行更改，全部排除于本 Slice scope；不能复用旧“仅 4 项”快照，也不能把
  dirty working tree 冒充 source candidate。旧 `frozen base` 只作为 `historical_fork_base` 保存。
- **V0 判定**：manifest digest `4effb1b2…`、binding digest `78a2bf95…` 与 independent
  verdict 已闭合，故 source/formal V0 为 DONE。详见
  `../../audits/2026-07-19-v0-v2-release-closure.md`。

**V0 历史交付记录（§8.2，2026-07-17；保留用于追溯）：**

- **HQA commit/branch**：`codex/full-9h` @ `5775d9a`（`3a95dd0` 已 fully merged，`merge-base = 3a95dd0`；
  其上修复 `5775d9a` = contract.py astimezone `OverflowError` fail-closed + 回归测试）。`historical_fork_base`
...
- **当时未清零、现已关闭的 blocker**：三仓 candidate/manifest 一致性与 primary validation binding；
  已由 2026-07-19 V0/V2 closure bundle + independent `CLEAR` 收口。
...
**V1 对抗复核与 source 收口（2026-07-19，platform
`codex/agent-v0-2-platform-v1@bb62f5d0bc6a`，已推送）：**
...
- clean controlled `bb62f5d` fresh platform full suite：`1573 passed, 99 skipped`；独立
...
- platform 主工作树仍含独立用户/并行更改，全部排除于本次 source scope，未处置、未回滚。正式
  manifest 必须精确声明生成时的 tracked/untracked 快照；不能沿用旧的固定 dirty 项数。上述 V1
  remediation source 已 commit/push，但仍不等于 live DB role/RLS release。
...
formal closure 冻结 source 坐标：HQA `codex/full-9h@4fdad9e7b31b`；platform
`codex/agent-v0-2-platform-v1@bb62f5d0bc6a`（已推送）；Hermes integration
...
formal validation binding：Hermes exact focused 15 files `389 passed`；HQA V0/V2 exact final-head
8 files `733 passed`；platform clean controlled full `1573 passed, 99 skipped`。三份 JUnit hash/count/
source coordinate 已绑定到同一 identity manifest，并由 independent reviewer fresh 重算 remote、tree、
archive 后 `CLEAR`。较早的 408/436/1531 结果只保留为上下文，不冒充 current primary evidence。
测试无 provider、paper/live、broker、Gate、redirect 或交易调用。
...
证据为主。本轮未声称 Hermes 全仓 suite 全绿；upstream 约 40k 的全仓 suite 明确不是本计划的
release gate，也未授权 live candidate install/canary。Hermes 更新频率由操作者决定，采用
periodic/manual update + no-agent watcher：watcher 只报告漂移，不得 install、restart、merge 或改变 gate。
...
| V2.7 approval challenge TTL / single-use / CAS | DONE | fake+live digest mismatch / expired 不消费；**V2.14 B1** resolve 失败 `release_approval_consume` |
| V2.8 idempotent stop + restart reconcile | DONE | fake+live stop 幂等跨重启；**V2.14 B2** 已 terminal 时回报实际 `succeeded`/`failed`/`stopped`（不 coerce→stopped） |
...
```
tests/test_hermes_run_adapter.py          50   (+2 structural mutator gate)
tests/test_hermes_run_acceptance.py       19
tests/test_hermes_run_acceptance_live.py  13   (stop idempotency asserts actual terminal)
合计                                       82 green
```
...
- OfficialHermesHttpAdapter **结构上** 在 mutator 路径强制上述 gate（V2.12-C），不只靠调用方自觉
- **未** 启用 live durable ON；**未** 触发 provider / paper / live / broker / Gate / redirect
- 用户当前 Discord 入口保留；失败路径有 stamp 内 rollback 步骤
- live launchd 仍在 `916f5fbf5`（B1/B2 代码在 integration，未强制 re-install）；isolated live suite 已用 `f44ec3afb` 覆盖 B1/B2
...
2. **状态名 completed/cancelled vs succeeded/stopped**：store 与 broker 双名兼容（`_broker_is_terminal` 已含两侧）；对外契约以 API 返回值为准。V2.14 B2 后 stop 诚实回报 store terminal，不再 coerce。
3. **Approval TTL 白盒**：live 用加速时钟/短 TTL 路径验证过期不消费；生产默认 300s 未做 wall-clock 长等。
4. **Live durable ON canary**：install 刻意 flag OFF。打开需**单独 canary 授权**（备份→ON→短冒烟→OFF/rollback）。当前 gate + structural mutator gate 保证 OFF 时 dispatch 关闭。
...
9. **Live launchd 未含 V2.14**：production 进程仍 `916f5fbf5`；B1/B2 在 integration + isolated suite。下次受控 re-install（durable 仍 OFF）可并入，**不** 自动执行。
...
- 当时未启动 V3；该历史边界现已由下方 V3 acceptance 取代

### Slice V3 — HQA Intent / Workflow Authority 深化（C3 foundation）

**状态（2026-07-19）：DONE — SOURCE ACCEPTED / LOCAL DARK INSTALL COMPLETE。** V3 source
commit 为 `121926388d864e02c0c1d380cd4e828158b3f380`，已推送。production AES-GCM/Keychain
intent、TTL/tombstone/ack、Task `1:N` Attempt、CAS/replay/backup、crash-convergent binding、只读
Hermes skill/wrapper 与 local `--no-agent` retention 均已交付。targeted `218 passed`；显式禁用
isolated real-Hermes suite 的 HQA full 为 `1726 passed, 15 skipped`；Ruff、py_compile、bash -n 与
diff check 全绿。expiry authority re-review 和全 V3 independent adversarial review 都 `CLEAR`，无
P0/P1/P2。安装未创建 Keychain key/authority root，未重启 Hermes；唯一 cron ID
`cb5702cc3909`。完整证据与 residual 见
`../../audits/2026-07-19-agent-v0-2-v3-acceptance.md`。

V3 DONE 仅代表内部暗态地基完成；public composer、Web mutation、Hermes Run、worker
claim/dispatch、provider、数据库与交易链路仍 OFF。
...
| V7a-Hermes-Approval-Decide | **M1 ACCEPT@2026-07-22** | hermetic `CommandApprovalAuthority` + typed `hermes.command_approval.decide` via `/act`；CAS = approval_ref+run_ref+digest+pending+expires_at；`allow_once|deny` only；single-use + same-action idempotent；fail-closed stale/expired/digest/run；snapshot `command_approval=ready` + empty-honest `approvals[]`；FE `data-hermes-approval-decide=v7a-m1` gated by `mutation_enabled`；**无 always-allow**；≠ Gate 1/2/3；≠ live Hermes projector |
| V7b-Hermes-Approval-Release | **M1 ACCEPT@2026-07-22** | hermetic `FakeHermesApprovalReleaseAdapter` + `project_pending_challenge` dual-seed；saga post-CAS `respond_approval`；`allow_once→once` / `deny→deny`；events `approval.responded|release_committed|signalled`；idempotent replay 不双 signal；release fail → `reconciling`（CAS 已提交、pending 不复活）；**无** always-allow / Gate / stop / live HTTP / `import hqa` |
| V7c-Hermes-Stop | **M1 ACCEPT@2026-07-22** | hermetic `FakeHermesRunStopAdapter` + typed `run.stop.request`/`RequestStop`；terminal-honest already_terminal；`run.cancelled`→stopped；client_action_id+digest 幂等；`partial_stop`→`reconciling` 后 replay 自愈；receipt `stop_layers`（§5.5）；unknown Attempt → overall `reconciling`（不发明 Task stopped）；**无** FE stop / Gate / live HTTP / `import hqa` / public write |
...
| V7g-B-M5 hermetic Gate2 seed cascade | **M5 ACCEPT@2026-07-23**（platform `ecec75e`） | typed act `vertical.factor_b.gate2_seed` CAS on gate1_confirmed Task（task_ref + expected_bind/plan digests + expected_gate1_id + expected_gate1_confirm_digest + expected_candidate_digest）→ `cascade_stage=gate2_seeded` + pending Gate2 on `gates[]`；new attempt/run/result；keeps `gate_cascade_locked`；always sample；**Not** Gate2 decide/StartResearch/orders/Git/live；V7e review surface-only while cascade stays locked；race-safe sole-owner+cascade lock；M5 ≠ M6 auth |
| Plan-V6-Token-Stream-M1 | **ACCEPT@2026-07-23**（platform `5788379`） | body-free SSE `event:transcript` hints + FE phase machine + spine-refetch messages BFF；markers `data-hermes-token-stream=v6-m1`；limitations honesty；**Not** provider-token passthrough / public write / M6 / Task invention |
| V8-M1 Adversarial Acceptance Prep | **ACCEPT@2026-07-23**（docs-only `f17c27b`） | audit matrix TC-01…29 + G01…G08 + GAP-01…17 + suite inventory；Architect FREEZE GO；`release_authorized=false`；**Not** release/canary/public/M6 |
...
| V8-M4 independent security+code CLEAR | **CLEAR_WITH_NITS@2026-07-23**（platform freeze `b570f94`；docs this session） | G4 checklist PASS（CSRF/origin、intent crypto、approval/Gate digests、no always-allow、Gate3 no web commit、provider evidence、kill_switch default true、hermetic chat_write OFF、process boundary）；bound campaign platform **357 passed / 5 skipped** + HQA **271 passed** + FE **51 vitest**；verdict `docs/audits/evidence/2026-07-23-v8-m4-independent-clear-verdict.json`；residuals GAP-01/03/04/07/10/12/13 honest；**Not** canary/public/M6/release；NEXT **V8-M5 canary（USER AUTH REQUIRED）** |
...
- transcript + assistant stream、Task drawer、plan/step、typed result canvas、provider/usage、
  ...
  V7a–V7g-B-M5 ACCEPT；**Plan-V6-Token-Stream-M1 ACCEPT@5788379**；**V8-M1 prep ACCEPT**；M6 Gate2 decide 仍未做；**V8-M2 ACCEPT_WITH_NITS@f5d41f4**；**V8-M3 ACCEPT@b570f94**；**V8-M4 CLEAR_WITH_NITS**；NEXT V8-M5 canary（USER AUTH REQUIRED）。
...
**进度（2026-07-23）：** `V7a`–`V7g-B-M5` **ACCEPT** — command-approval decide/release/stop/projector + Domain Gate 1/2/3 surfaces + typed results on spine + **Vertical A bind**（hermetic + authorized live Futu RO）+ **Vertical B hermetic factor bind + plan-confirm + Gate1 seed + Gate1 confirm + Gate2 seed**（`vertical.factor_b.bind` → `plan_confirm` → `gate1_seed` → `gate1_confirm` → `gate2_seed` CAS → `cascade_stage=gate2_seeded` + pending Gate2；`gate_cascade_locked` stays；no auto Gate2 decide）。无 always-allow；无 public write；不发明 Task from conversation.turn。**Plan-V6-Token-Stream-M1 ACCEPT@5788379**（hints-only SSE + spine-refetch）。其余：**V8-M1 prep ACCEPT**；**V8-M2 ACCEPT_WITH_NITS@f5d41f4**；**V8-M3 ACCEPT@b570f94**；**V8-M4 CLEAR_WITH_NITS**（G4 surface；`release_authorized=false`）；**NEXT V8-M5 canary（USER AUTH REQUIRED）**；**M6 Gate2 decide couple 未授权**；public write OFF；full V8 release gates / canary–public cutover NOT STARTED（G4 surface CLEAR_WITH_NITS only）。
...
| V6 final workspace UI | **PARTIAL（token-stream M1 ACCEPT@5788379；V8-M1–M3 ACCEPT + M4 CLEAR_WITH_NITS）** | 本地 dark + L2a–L5c + V7a–V7g-B-M5 ACCEPT；**Plan-V6-Token-Stream-M1 ACCEPT**（hints + spine-refetch；≠ full Plan-V6 close）；public chat 仍 OFF；V8-M1 ACCEPT + V8-M2 ACCEPT_WITH_NITS@f5d41f4 + V8-M3 ACCEPT@b570f94 + **V8-M4 CLEAR_WITH_NITS**；NEXT V8-M5 canary（USER AUTH） |
| V7 decisions/results/vertical slices | **PARTIAL（V7a–V7g-B-M5 ACCEPT@2026-07-22…23；token-stream M1 ACCEPT@5788379；V8-M1–M3 ACCEPT + M4 CLEAR_WITH_NITS）** | decide/release/stop/projector/Gate/typed-results/Vertical A+B M1–M5 + token-stream M1 green；**M6 Gate2 decide 未授权**；V8-M1 ACCEPT + V8-M2 ACCEPT_WITH_NITS + V8-M3 ACCEPT@b570f94 + **V8-M4 CLEAR_WITH_NITS**；NEXT V8-M5 canary（USER AUTH） |
| V8 adversarial acceptance/release | **PARTIAL（M1–M4；G4 CLEAR_WITH_NITS）** | M1 prep + M2 suite + M3 cold-start + **M4 independent CLEAR_WITH_NITS** done；canary/public/user acceptance + one-time cutover **NOT STARTED**（need user auth）；`release_authorized=false` |
...
8. **V6 本地 dark + L2a–L5c thin rail ACCEPT（2026-07-21…22）**：真实 HTTP adapter、
   ...
   **Plan-V6 完整 UI 仍 PARTIAL；V7a–V7g-B-M5 decide/release/stop/projector/Gate-surfaces/typed-results/hermetic+authorized-live-RO Vertical A + hermetic Vertical B bind+plan-confirm+Gate1 seed+Gate1 confirm+Gate2 seed ACCEPT。** 当前施工入口：**V8-M5 canary grant（USER AUTH REQUIRED）**（V8-M1–M3 ACCEPT@f17c27b/`f5d41f4`/`b570f94`；**V8-M4 CLEAR_WITH_NITS**@freeze `b570f94`；Plan-V6-Token-Stream-M1 ACCEPT@5788379；still zero orders；kill_switch true；public write OFF；StartResearch 仍 dark；gate_cascade_locked stays；**M5 ≠ M6 auth；token-stream ≠ M6 auth；M1–M4 ≠ release/canary；M4 CLEAR ≠ M5 auth**）；public chat 始终 OFF 直至 full V8 gates。
