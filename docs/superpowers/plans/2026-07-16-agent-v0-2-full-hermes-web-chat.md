...
> **状态（2026-07-22）：CURRENT / PLAN ACCEPTED / V0–V5 DONE（V2 live durable OFF）/ V6 LOCAL DARK ENABLEMENT ACCEPT@2026-07-21 / L2a–L5c thin rail ACCEPT@2026-07-22 / V7a–V7d decide/release/stop/projector M1 ACCEPT@2026-07-22 / Plan-V6 FULL UI PARTIAL / NEXT V7e+（Gate surfaces；public write 仍 OFF）。** 本计划是 D-32 唯一 active
...
> token stream / launchd 常驻 / public V8 仍未做（V7a–V7d decide/release/stop/projector ACCEPT；V7e+ OPEN）。
>
...
| Plan-V6 完整 UI | **PARTIAL / NEXT** | assistant token stream 等仍未做；a11y M1 + V7a–V7d decide/release/stop/projector ACCEPT；V7e+ OPEN |
...
- transcript + assistant stream、Task drawer、plan/step、typed result canvas、provider/usage、
  ...
  V7a–V7d decide/release/stop/projector ACCEPT；token stream / V7e+ 仍未做。
...
**进度（2026-07-22）：** `V7a` decide + `V7b` release + `V7c` stop + `V7d` durable approval projector **M1 ACCEPT** — hermetic authority + `allow_once|deny` CAS via `/act` + post-CAS hermetic `respond_approval` release/signal + FE panel + hermetic Run-scoped stop with §5.5 layered receipt + pending/decided projection on snapshot/follow/SSE spine（`list_observed` + fingerprint-gated `event:approvals`；marker `v7d-m1`）；无 always-allow；无 Gate/token stream；stop/projector 不发明 Task。其余 V7（**V7e Gate surfaces**、typed results、两纵切）仍 OPEN。
...
| V6 final workspace UI | **PARTIAL（2026-07-22）** | 本地 dark enablement + L2a–L5c thin rail ACCEPT（含 L5b/L5c）；V7a–V7c decide/release/stop ACCEPT；token stream 仍 NEXT；public chat 仍 OFF |
| V7 decisions/results/vertical slices | **PARTIAL（V7a–V7d M1 ACCEPT@2026-07-22）** | exact command-approval decide + hermetic release/signal + hermetic layered stop + durable approval projector green；**V7e Gate surfaces** / results / verticals 仍 OPEN |
...
8. **V6 本地 dark + L2a–L5c thin rail ACCEPT（2026-07-21…22）**：真实 HTTP adapter、
   ...
   **Plan-V6 完整 UI 仍 PARTIAL；V7a–V7d decide/release/stop/projector ACCEPT。** 当前施工入口：**V7e Gate surfaces** → V7f typed results → V7g verticals；public chat 始终 OFF 直至 V8。
