...
| V7g-A-M1 hermetic vertical binding | **M1 ACCEPT@2026-07-23** | NL goal → typed act `vertical.options_a.bind` → Task/Attempt/Run + V7f typed options result → `completed`\|`completed_degraded`（evidence present vs omitted）；receipt carries task/attempt/run/result ids + terminal；snapshot projects ids + linked result；`authority_health.task|attempt|run=ready`（empty honest）；idempotent client_action_id+digest；mutation OFF fail-closed；always sample + `not_live_futu_quote`；**Not** StartResearch（still dark）；**无** live Futu / orders / public write / Task invention from conversation.turn / `import hqa` |
