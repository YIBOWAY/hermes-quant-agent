...
> **状态（2026-07-22）：CURRENT / PLAN ACCEPTED / V0–V5 DONE（V2 live durable OFF）/ V6 LOCAL DARK ENABLEMENT ACCEPT@2026-07-21 / L2a–L5c thin rail ACCEPT@2026-07-22 / V7a-Hermes-Approval-Decide M1 ACCEPT@2026-07-22 / Plan-V6 FULL UI PARTIAL / NEXT V7b+（public write 仍 OFF）。** 本计划是 D-32 唯一 active
...
> token stream / launchd 常驻 / public V8 仍未做（V7a decision mutation ACCEPT；V7b+ OPEN）。
>
...
| Plan-V6 完整 UI | **PARTIAL / NEXT** | assistant token stream 等仍未做；a11y M1 + V7a decision mutation ACCEPT；V7b+ OPEN |
...
- transcript + assistant stream、Task drawer、plan/step、typed result canvas、provider/usage、
  ...
  V7a decision mutation ACCEPT；token stream / V7b+ 仍未做。
...
**进度（2026-07-22）：** `V7a-Hermes-Approval-Decide M1` **ACCEPT** — hermetic authority + `allow_once|deny` CAS via `/act` + FE panel；无 always-allow；无 Gate/stop/token stream。其余 V7（stop、Gate surfaces、durable Hermes projector、typed results、两纵切）仍 OPEN。
...
| V6 final workspace UI | **PARTIAL（2026-07-22）** | 本地 dark enablement + L2a–L5c thin rail ACCEPT（含 L5b/L5c）；V7a decision mutation ACCEPT；token stream 仍 NEXT；public chat 仍 OFF |
| V7 decisions/results/vertical slices | **PARTIAL（V7a M1 ACCEPT@2026-07-22）** | exact command-approval decide green；stop / Gate surfaces / durable projector / results 仍 OPEN |
...
8. **V6 本地 dark + L2a–L5c thin rail ACCEPT（2026-07-21…22）**：真实 HTTP adapter、
   ...
   **Plan-V6 完整 UI 仍 PARTIAL；V7a decide ACCEPT。** 当前施工入口：V7b+（stop/Gates/results/durable approval projector）；public chat 始终 OFF 直至 V8。
