...
> **状态（2026-07-23）：CURRENT / PLAN ACCEPTED / V0–V5 DONE（V2 live durable OFF）/ V6 LOCAL DARK ENABLEMENT ACCEPT@2026-07-21 / L2a–L5c thin rail ACCEPT@2026-07-22 / V7a–V7g-B-M1 decide/release/stop/projector/Gate-surfaces/typed-results/hermetic+live-RO Vertical A + hermetic Vertical B factor bind ACCEPT@2026-07-22…23 / Plan-V6 FULL UI PARTIAL / NEXT V7g-B-M2+ narrow cascade（public write 仍 OFF）。** 本计划是 D-32 唯一 active
...
> token stream / launchd 常驻 / public V8 仍未做（V7a–V7g-B-M1 decide/release/stop/projector/Gate-surfaces/typed-results/hermetic+authorized-live-RO Vertical A + hermetic Vertical B factor bind ACCEPT；V7g-B-M2+ cascade OPEN）。
>
...
| Plan-V6 完整 UI | **PARTIAL / NEXT** | assistant token stream 等仍未做；a11y M1 + V7a–V7g-B-M1 ACCEPT；V7g-B-M2+ cascade OPEN |
...
- transcript + assistant stream、Task drawer、plan/step、typed result canvas、provider/usage、
  ...
  V7a–V7g-B-M1 decide/release/stop/projector/Gate-surfaces/typed-results/hermetic+authorized-live-RO Vertical A + hermetic Vertical B factor bind ACCEPT；token stream / V7g-B-M2+ cascade 仍未做。
...
**进度（2026-07-23）：** `V7a`–`V7g-B-M1` **ACCEPT** — command-approval decide/release/stop/projector + Domain Gate 1/2/3 surfaces + typed results on spine + **Vertical A bind**（hermetic + authorized live Futu RO）+ **Vertical B hermetic factor bind**（`vertical.factor_b.bind` → Task/Attempt/Run + typed `factor` sample result；cascade locked）。无 always-allow；无 public write；不发明 Task from conversation.turn。其余 V7（**V7g-B-M2+ cascade**）仍 OPEN。
...
| V6 final workspace UI | **PARTIAL（2026-07-23）** | 本地 dark enablement + L2a–L5c thin rail ACCEPT（含 L5b/L5c）；V7a–V7g-B-M1 decide/release/stop/projector/Gate-surfaces/typed-results/hermetic+live-RO Vertical A + hermetic Vertical B factor bind ACCEPT；token stream 仍 NEXT；public chat 仍 OFF |
| V7 decisions/results/vertical slices | **PARTIAL（V7a–V7g-B-M1 ACCEPT@2026-07-22…23）** | command-approval decide/release/stop/projector + Gate surfaces + typed results + Vertical A hermetic+live RO + Vertical B hermetic factor bind green；**V7g-B-M2+ cascade** 仍 OPEN |
...
8. **V6 本地 dark + L2a–L5c thin rail ACCEPT（2026-07-21…22）**：真实 HTTP adapter、
   ...
   **Plan-V6 完整 UI 仍 PARTIAL；V7a–V7g-B-M1 decide/release/stop/projector/Gate-surfaces/typed-results/hermetic+authorized-live-RO Vertical A + hermetic Vertical B factor bind ACCEPT。** 当前施工入口：**V7g-B-M2+ narrow cascade**（still zero orders；kill_switch true；public write OFF；StartResearch 仍 dark）→ optional token stream → V8；public chat 始终 OFF 直至 V8。
