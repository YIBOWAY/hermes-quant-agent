...
- **V4–V6 / L2a / L2b (2026-07-21…22) status for agents:** V4 live schema ACCEPT
  ...
  marker `l5c-m1`). **V7a–V7g-A-M2 ACCEPT** (exact allow_once|deny CAS + hermetic respond_approval release/signal + hermetic Run-scoped stop with §5.5 layered receipt + durable approval projector + Domain Gate 1/2/3 surfaces + typed results on spine + Vertical A options bind hermetic fixture + authorized live Futu RO thin overlay → Task/Attempt/Run + typed result → completed|completed_degraded; real only with verifiable provider_evidence; RO façade no trade/account; sample/real fail-closed; no always-allow; Gates ≠ command-approval ≠ results; no Task invention from conversation.turn; no dual private poll; no catalog-on-spine; zero orders; kill_switch true). Plan-V6 remainder (token stream) + V7g-B + public V8 still open.
