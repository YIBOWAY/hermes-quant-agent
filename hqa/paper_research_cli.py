...
code, executes a provider, places an order, commits Git changes, or stores paper
text / prompts / source bytes.
...
def _start_plan(
    ...
    *,
    ...
    binding = {
        ...
        "invocation_attestation_ref": invocation["attestation_ref"],
        "command_id": subject["command_id"],
        "hermes_run_id": subject["hermes_run_id"],
    ...
    receipt = authority.apply(  # type: ignore[attr-defined]
        StartResearch(
            operation_id=_operation_id(outer, "start", binding),
...
def _confirm_plan(
    ...
    *,
    ...
    binding = {
        ...
        "runtime_command_id": request["command_id"],
        "runtime_hermes_run_id": request["hermes_run_id"],
        "runtime_hermes_session_id": request["hermes_session_id"],
        "runtime_platform_session_id": request["platform_session_id"],
        "invocation_attestation_ref": invocation["attestation_ref"],
    }
    receipt = authority.apply(  # type: ignore[attr-defined]
        ConfirmPlan(
...
def _open_gate3(
    ...
    *,
    ...
    if (
        ...
        or final_backtest.get("final_backtest_provider") != "futu"
    ):
        raise _OperationError(
            "paper_research_final_receipt_invalid",
            retryable=False,
    ...
    existing = _show_or_none(registry, gate_id)
    if existing is not None:
        registration = {field: existing.get(field) for field in _REGISTER_FIELDS}
    ...
    receipt = authority.apply(  # type: ignore[attr-defined]
        ContinueResearch(
            _operation_id(outer, "research-continue", binding),
            task_ref,
            expected,
            payload_ref,
            intent_expires_at,
...
def _complete_after_human_commit(
    ...
    *,
    ...
    if (
        ...
        or final_backtest.get("final_backtest_provider") != "futu"
        or any(gate.get(field) != value for field, value in final_backtest.items())
    ):
        raise _OperationError(
